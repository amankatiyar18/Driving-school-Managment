...
?>
    ...
    <!-- /.content -->
      ...
      <div class="card">
        <div class="card-body">
<center><input type="button" name="submit" id="submit" value="Click Here to End" class="btn btn-info" onclick="confirmtocomplete('<?php echo $_SESSION[quiztopicid]; ?>','<?php echo $_SESSION[studentid]; ?>')" ></center>
        </div>
        <!-- /.card-body -->
      </div>
      <!-- /.card -->
...
include("footer.php");
?>
<div id="idansstatus"><?php $sqlqz ="SELECT * FROM quiz_result WHERE quiztopicid='$_SESSION[quiztopicid]' AND date='$_SESSION[dttim]' AND studentid='$_SESSION[studentid]' and selectedanswer != ''";
$qsqlqz  = mysqli_query($con,$sqlqz); ?><input type="hidden" name="answereedq" id="answereedq" value="<?php echo mysqli_num_rows($qsqlqz); ?>" ><?php $sqlqz ="SELECT * FROM quiz_result WHERE quiztopicid='$_SESSION[quiztopicid]' AND date='$_SESSION[dttim]' AND studentid='$_SESSION[studentid]' and selectedanswer=''";
...
<?php
//if($rsedit['qdisptype'] == "Pagination Viewer" )
{
?>
<script>
...
function confirmtocomplete(quiztopicid,studentid) 
{
	if(document.getElementById("unanswereedq").value == 0)
	{
		...
		{
			window.location="quiz_result.php?quiztopicid="+quiztopicid+"&studentid="+studentid;
		}
	}
	if(document.getElementById("unanswereedq").value != 0)
	{
